...
class PromptPerformanceMonitor:
                ...
                cls.metrics["performance_history"].append(
                    {
                        ...
                        "cache_hit": (
                            kwargs.get("cache_hit", False) if "cache_hit" in kwargs else None
                        ),
                    }
                )
    ...
    def get_report(cls) -> dict[str, Any]:
        ...
        if build_times:
            ...
            p95_build_time_ms = (
                statistics.quantiles(build_times, n=20)[18] * 1000
                if len(build_times) >= 20
                else None
            )
        else:
    ...
    def calculate_improvement(cls) -> dict[str, Any]:
        ...
        if baseline_avg_build_time_ms > 0:
            build_time_improvement = (
                (baseline_avg_build_time_ms - current_avg_build_time_ms)
                / baseline_avg_build_time_ms
            ) * 100

        length_reduction = 0
        if baseline_avg_length > 0:
            length_reduction = (
                (baseline_avg_length - current_avg_length) / baseline_avg_length
            ) * 100

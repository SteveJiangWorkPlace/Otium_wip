...
if DEFAULT_TEMPLATE_VERSION != "production":
    ...
    print("   [PASS] DEFAULT_TEMPLATE_VERSION 正确")
...
if TRANSLATION_TEMPLATE_VERSION != "production":
    ...
    print("   [PASS] TRANSLATION_TEMPLATE_VERSION 正确")
...
if ENGLISH_REFINE_TEMPLATE_VERSION != "production":
    ...
    print("   [PASS] ENGLISH_REFINE_TEMPLATE_VERSION 正确")
...
if DEFAULT_ANNOTATIONS_VERSION != "production":
    ...
    print("   [PASS] DEFAULT_ANNOTATIONS_VERSION 正确")
...
# 测试翻译提示词
print("   测试学术翻译提示词...")
original_translation_prompt = build_academic_translate_prompt_original(
    test_text, style="US", version="professional"
)
current_translation_prompt = build_academic_translate_prompt(
...
if "灵活表达" in original_annotations:
    print("   [FAIL] 原始版本仍包含'灵活表达'（应已移除）")
    config_correct = False
else:
    print("   [PASS] 原始版本已移除'灵活表达'")
...
# 检查是否包含原始版本中的关键例子
has_examples = (
    "Find: I will" in current_human
    and "Replace with: I hope to" in current_human
    and "Find: utilize, employ" in current_human
    and "Replace with: use, make use of" in current_human
    and "Use contractions" in current_human
)

if has_examples and 1000 < len(current_human) < 2000:
    print("   [PASS] '人性化处理'已更新并融入原始例子")
    print(f"      原始长度: {len(original_human)} 字符")
    print(f"      当前长度: {len(current_human)} 字符")
else:
    print("   [FAIL] '人性化处理'可能未正确更新")
    print(f"      原始长度: {len(original_human)} 字符")
    print(f"      当前长度: {len(current_human)} 字符")
    if not has_examples:
        print("   [INFO] 缺少原始版本中的关键例子")
...
for key in sorted(current_annotations.keys()):
    preview = (
        current_annotations[key][:50] + "..."
        if len(current_annotations[key]) > 50

...
def test_basic_functionality():
    ...
    print(f"   长度减少: {((len(original_prompt) - len(compact_prompt)) / len(original_prompt) * 100):.1f}%")

    # 优化版本 (ai_optimized)
    ai_prompt = build_error_check_prompt(test_text, template_version="ai_optimized")
    print(f"   AI优化版本长度: {len(ai_prompt)} 字符")
    print(f"   长度减少: {((len(original_prompt) - len(ai_prompt)) / len(original_prompt) * 100):.1f}%")
    ...
    print(f"   长度减少: {((len(original_trans_prompt) - len(opt_prompt_1)) / len(original_trans_prompt) * 100):.1f}%")

    # 优化版本 - 第二次（从缓存读取）
    start_time = time.time()
    opt_prompt_2 = build_academic_translate_prompt(test_text, use_cache=True)
    time_2 = time.time() - start_time
    print(f"   优化版本第二次构建时间: {time_2*1000:.2f}ms")
    print(f"   缓存加速: {((time_1 - time_2) / time_1 * 100):.1f}%" if time_1 > 0 else "   缓存加速: N/A")
    ...
    if key in original_ann:
        ...
        print(f"     Compact: {comp_len} 字符 (减少 {((orig_len - comp_len) / orig_len * 100):.1f}%)" if comp_len > 0 else "     Compact: N/A")
        print(f"     AI优化: {ai_len} 字符 (减少 {((orig_len - ai_len) / orig_len * 100):.1f}%)" if ai_len > 0 else "     AI优化: N/A")
    ...
    # 总结
    print("\n优化效果总结:")
    print(f"1. 纠错提示词长度减少: {((len(original_prompt) - len(ai_prompt)) / len(original_prompt) * 100):.1f}%")
    print(f"2. 翻译提示词长度减少: {((len(original_trans_prompt) - len(opt_prompt_1)) / len(original_trans_prompt) * 100):.1f}%")
    print(f"3. 缓存命中后构建时间减少: {((time_1 - time_2) / time_1 * 100):.1f}%" if time_1 > 0 else "3. 缓存命中后构建时间减少: N/A")

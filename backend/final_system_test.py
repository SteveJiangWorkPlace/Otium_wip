...
if DEFAULT_TEMPLATE_VERSION != "original":
    print(f"   [FAIL] DEFAULT_TEMPLATE_VERSION 应为 'original'，实际为 '{DEFAULT_TEMPLATE_VERSION}'")
    config_correct = False
else:
    print(f"   [PASS] 智能纠错使用原始版本")

if TRANSLATION_TEMPLATE_VERSION != "original":
    print(f"   [FAIL] TRANSLATION_TEMPLATE_VERSION 应为 'original'，实际为 '{TRANSLATION_TEMPLATE_VERSION}'")
    config_correct = False
else:
    print(f"   [PASS] 学术翻译使用原始版本")

if ENGLISH_REFINE_TEMPLATE_VERSION != "original":
    print(f"   [FAIL] ENGLISH_REFINE_TEMPLATE_VERSION 应为 'original'，实际为 '{ENGLISH_REFINE_TEMPLATE_VERSION}'")
    config_correct = False
else:
    print(f"   [PASS] 英文精修使用原始版本")

if DEFAULT_ANNOTATIONS_VERSION != "original_modified":
    print(f"   [FAIL] DEFAULT_ANNOTATIONS_VERSION 应为 'original_modified'，实际为 '{DEFAULT_ANNOTATIONS_VERSION}'")
    config_correct = False
else:
    print(f"   [PASS] 快捷批注使用修改后的原始版本")
...
if len(original_error) == len(current_error):
    ...
    print(f"   [FAIL] 智能纠错提示词长度不一致")
    print(f"      原始: {len(original_error)} 字符")
    print(f"      当前: {len(current_error)} 字符")
    config_correct = False

print(f"   测试学术翻译提示词...")
original_translation = build_academic_translate_prompt_original(test_text, style="US", version="professional")
current_translation = build_academic_translate_prompt(test_text, style="US", version="professional", template_version="original")

if len(original_translation) == len(current_translation):
    print(f"   [PASS] 学术翻译提示词长度一致: {len(original_translation)} 字符")
else:
    print(f"   [FAIL] 学术翻译提示词长度不一致")
...
if "灵活表达" in original_annotations:
    print(f"   [PASS] 原始版本包含'灵活表达'")
else:
    print(f"   [FAIL] 原始版本不包含'灵活表达'")
...
if "Revise the text to sound more like a thoughtful but less confident human" in current_human:
    print(f"   [PASS] '人性化处理'已更新为用户提供的新版本")
else:
    print(f"   [FAIL] '人性化处理'未正确更新")
    config_correct = False
...
if config_correct:
    ...
    print("   - 修改'符号修正': '确保标点符号在引号外，同时用分号连接两个关系紧密的独立从句' [PASS]")
    ...
    for key in sorted(current_annotations.keys()):
        preview = current_annotations[key][:80] + "..." if len(current_annotations[key]) > 80 else current_annotations[key]
        print(f"   - {key}: {preview}")
